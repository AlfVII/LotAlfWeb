...
class RetailerForm(FlaskForm):
    ...
    retailer_street_number = TextAreaField(u'Número de la calle', validators=[Optional()], render_kw={"rows": 1})
    retailer_postal_code = IntegerField(u'Código postal', validators=[Optional()])
    retailer_telephone = IntegerField(u'Número de telefono', validators=[Optional()])
    retailer_latitude = FloatField(u'Latitud', validators=[Optional()])
    retailer_longitude = FloatField(u'Lonfigud', validators=[Optional()])

    number = IntegerField(u'Número de lotería', validators=[RequiredIf(owned='Owned')])
